advent_data = ["27 29 30 33 34 35 37 35",
...
safe_reports = 0  # What to return

def check_interval(new_lvl, old_lvl)
    diff = (new_lvl - old_lvl).abs
    return (diff >= 1 && diff <= 3)
end

# Without Probleme Dampener
advent_data.each do |report|
    ref_direction = nil
    safe = true
    tmp_lvl = nil  # For comparison
    report.split(" ").each do |level|
        level = level.to_i
        if tmp_lvl != nil then
            # Check intervals
            if !check_interval(level, tmp_lvl) then
                safe = false
                break
            end

            # Check direction consistency
            if ref_direction == nil
                ref_direction = (level > tmp_lvl)? "ASC" : "DSC"
            else
                obs_direction = (level > tmp_lvl)? "ASC" : "DSC"
                if ref_direction != obs_direction
                    safe = false
                    break
                end
            end
        end
        tmp_lvl = level  # For next comparison
    end
...
# With probleme dampener
advent_data.each do |report|
    ref_direction = nil
    safe = true
    tmp_lvl = nil  # For comparison
    possible_problems_idx = -1
    ...
    report.each_with_index do |level, idx|
        ...
        if tmp_lvl != nil then
            ...
            if !check_interval(level, tmp_lvl) then
                safe = false
                possible_problems_idx = idx
                break
            end

            # Check direction consistence
            if ref_direction == nil
                ref_direction = (level > tmp_lvl)? "ASC" : "DSC"
            else
                obs_direction = (level > tmp_lvl)? "ASC" : "DSC"
    ...
    p "False found : " + report[possible_problems_idx-1].to_s + " " + report[possible_problems_idx].to_s
    
    # Second pass without first

    # Duplicate then performs the deletion, then rerturns the chaged array
    report_retest1 = report.dup.tap{|a| a.delete_at(possible_problems_idx-1)}

    p "Trying " + report_retest1.join(" ")

    ref_direction = nil
    safe = true  # It's safe again !
    tmp_lvl = nil
    report_retest1.each do |level|
        level = level.to_i
        if tmp_lvl != nil then
            # Check intervals
            if !check_interval(level, tmp_lvl) then
    ...
    # Third pass without second possibility
    p "Still unsafe ..."
    # Duplicate then performs the deletion, then rerturns the chaged array
    report_retest2 = report.dup.tap{|a| a.delete_at(possible_problems_idx)}

    p "Trying " + report_retest2.join(" ")

    ref_direction = nil
    safe = true  # It's safe again !
    tmp_lvl = nil
    report_retest2.each do |level|
        level = level.to_i
        if tmp_lvl != nil then
            # Check intervals
            if !check_interval(level, tmp_lvl) then
    ...
    if possible_problems_idx - 2 >= 0
        report_retest3 = report.dup.tap{|a| a.delete_at(possible_problems_idx-2)}
